...
class SwinTransformerSys3D(nn.Module):
    ...
    def __init__(self, pretrained=None,
        ...
        # self.mean_conv = nn.Conv3d(self.hidden_dim, 3, kernel_size=1, bias=False)
        # self.std_conv = nn.Conv3d(self.hidden_dim, 3, kernel_size=1, bias=False)
        ...
        # self.up_u = FinalPatchExpand_X4(input_resolution=(
        #     img_size[0] // patch_size[0], img_size[1] // patch_size[1], img_size[2] // patch_size[2]),
        #     dim_scale=4, dim=embed_dim)
        # self.output_u = nn.Conv3d(in_channels=embed_dim, out_channels=32, kernel_size=1, bias=False)
        #
        # self.input_proj = nn.Conv3d(in_channels=32, out_channels=32, kernel_size=1, bias=False)
        #
        # self.mean_conv = nn.Conv3d(in_channels=32, out_channels=3, kernel_size=1, bias=False)

        # self.std_conv = nn.Conv3d(in_channels=32, out_channels=3, kernel_size=1, bias=False)
    ...
    def forward_features(self, x):
        if x.shape[1] == 5:
            ...
            x[:, a, :, :, :] *= rand_mask.to(torch.float32)
    ...
    def reparameterize(self, mu, logvar, k=1):
        ...
        logvar = torch.unsqueeze(logvar, dim=1)
        for _ in range(k):
    ...
    def forward(self, x):
        ...
        # uncertainty
        # u = self.input_proj(u)
        # mean = self.mean_conv(u)
        # std = self.std_conv(u)
        #
        # u = self.reparameterize(mean, std, 1)
