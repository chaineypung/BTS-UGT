...
from TranBTS.TransBTS.TransBTS_downsample8x_skipconnection import TransBTS
import warnings
...
def main(args):
    ...
    args.save_folder_1 = pathlib.Path(f"./runs_TransBTS/{args.exp_name}/model_1")
    ...
    args.checkpoint_folder = pathlib.Path(f"./runs_TransBTS/{args.exp_name}/model_1")
    ...
    _, model_1 = TransBTS(dataset='brats', _conv_repr=True, _pe_type="learned")
    ...
    model_1 = model_1.cuda(0)

    model_file = args.save_folder_1 / "model.txt"
    with model_file.open("w") as f:
        print(model_1, file=f)

    criterion = EDiceLoss().cuda(0)
    criterian_val = EDiceLoss_Val().cuda(0)
    ...
    for epoch in range(args.epochs):
        try:
            ...
            for i, batch in enumerate(zip(train_loader)):
                ...
                inputs_S1, labels_S1 = inputs_S1.cuda(0), labels_S1.cuda(0)
    ...
    for i, val_data in enumerate(data_loader):
        ...
        with torch.no_grad():
            val_inputs, val_labels = (
                val_data["image"].cuda(0),
                val_data["label"].cuda(0),
            )
